...
const s = {
  ...
  aiBox: { background: "linear-gradient(135deg,#eff6ff,#f0fdf4)",
    border: "1px solid #bfdbfe", borderRadius: 12, padding: 20, marginBottom: 20 },
  aiHeader: { display: "flex", alignItems: "center", gap: 10, marginBottom: 16 },
  aiTitle: { fontSize: 14, fontWeight: 600, color: "#1e40af" },
  aiSub: { fontSize: 12, color: "#3b82f6" },
  aiBtn: { padding: "10px 20px", background: "#1d4ed8", color: "#fff",
    border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600,
    cursor: "pointer", display: "flex", alignItems: "center", gap: 8,
    whiteSpace: "nowrap" },
  aiBtnLoading: { background: "#93c5fd", cursor: "not-allowed" },
  specGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 },
  specItem: { background: "#fff", border: "1px solid #bfdbfe", borderRadius: 8,
    padding: "10px 14px" },
  ...
  uploadArea: { border: "2px dashed #d1d5db", borderRadius: 10, padding: "32px 20px",
    textAlign: "center", cursor: "pointer", transition: ".15s", background: "#fafafa" },
  uploadAreaHover: { borderColor: "#1d4ed8", background: "#eff6ff" },
  photoGrid: { display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginTop: 14 },
  photoItem: { position: "relative", aspectRatio: "4/3", borderRadius: 8,
...
export default function PublishCar() {
  ...
  const fetchSpecs = async () => {
    if (!form.brand || !form.model || !form.year) {
      setError("Completá marca, modelo y año antes de obtener las especificaciones.");
      return;
    }
    setError("");
    setAiLoading(true);
    try {
      ...
      setError("No se pudieron obtener las especificaciones. Verificá los datos del auto.");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const validateStep = () => {
    if (step === 0) {
      if (!form.brand || !form.model || !form.year || !form.location) {
        setError("Completá marca, modelo, año y ubicación para continuar.");
        return false;
      }
    }
    if (step === 1) {
      if (photos.length === 0) {
        setError("Subí al menos una foto del auto.");
        return false;
      }
    }
    if (step === 2) {
      if (!form.price_per_day) {
        setError("Ingresá el precio por día.");
        return false;
      }
      if (!ownerTerms) {
        setError("Tenés que aceptar las condiciones para dueños.");
        return false;
      }
    }
    setError("");
  ...
  if (done) return (
    <div style={s.page}>
      <div style={s.success}>
        ...
        <div style={s.successTitle}>¡Auto publicado y verificado!</div>
        <div style={s.successSub}>
          Las especificaciones fueron validadas automáticamente por IA.<br />
          Tu publicación ya está visible para los conductores.
        </div>
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.sub}>Completá los datos y dejá que la IA haga el resto</div>

      <div style={s.steps}>
        {STEPS.map((st, i) => (
          <div key={st} style={s.step}>
            <div style={{
              ...s.stepDot,
              ...(i < step ? s.stepDotDone : i === step ? s.stepDotActive : {})
            }}>
              {i < step ? "✓" : i + 1}
            </div>
            {st}
          ...
          <div style={s.grid2}>
            <div style={s.field}>
              ...
              <select style={s.select} value={form.category}
                ...
                {["Sedan", "SUV", "Pickup", "Deportivo", "Eléctrico", "Utilitario"].map(c =>
      ...
      {step === 1 && (
        <div style={s.card}>
          ...
          <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 16 }}>
            ...
            Incluí exterior, interior y baúl para generar más confianza.
          </p>
          ...
          <label
            ...
            <div style={{ fontSize: 13, color: "#9ca3af" }}>JPG, PNG — máximo 6 fotos</div>
          </label>
            ...
            <div style={s.photoGrid}>
              {photos.map((p, i) => (
                <div key={i} style={s.photoItem}>
                  <img src={p.url} alt={p.name} style={s.photoImg} />
                  {i === 0 && (
                    <div style={{ position: "absolute", bottom: 6, left: 6,
                      background: "#1d4ed8", color: "#fff", fontSize: 10,
      ...
      {step === 2 && (
        <div style={s.card}>
          ...
          <div style={s.grid2}>
            <div style={s.field}>
              ...
              <input style={s.input} type="number" placeholder="8500"
                value={form.price_per_day} onChange={e => set("price_per_day", e.target.value)} />
            </div>
            <div style={s.field}>
          ...
          <div style={s.termsBox}>
            <div style={s.termsTitle}>⚠️ Condiciones para dueños</div>
            <div style={s.termsText}>
              Al publicar tu auto en Freewheel aceptás que: (1) El vehículo debe contar
              con seguro vigente al momento de cada alquiler. (2) Debés subir la cédula
              verde/azul para verificación. (3) Freewheel retiene el pago hasta que el
              alquiler se complete sin incidentes. (4) Ante daños, el depósito de garantía
              actúa como primera cobertura antes de derivar al seguro.
            </div>
            <div style={s.termsCheck}>
              <input type="checkbox" checked={ownerTerms}
                onChange={e => setOwnerTerms(e.target.checked)} />
              <span style={{ fontSize: 13, color: "#92400e" }}>
                Entiendo y acepto estas condiciones como dueño del vehículo
              </span>
            </div>
          </div>
      ...
      {step === 3 && (
        <div style={s.card}>
          ...
          {[
            ...
            ["Specs IA", specs ? "✓ Completadas automáticamente" : "No obtenidas"],
          ].map(([k, v]) => (
          ...
          <div style={{ marginTop: 16, padding: 12, background: "#f0fdf4",
            ...
            ✅ Tu auto quedará <strong>verificado automáticamente</strong> gracias
            a la validación por IA. Estará visible al instante para los conductores.
          </div>
