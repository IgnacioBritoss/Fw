...
const s = {
  ...
  reportType: { fontSize: 11, fontWeight: 600, padding: "2px 10px",
    borderRadius: 20, marginBottom: 6, display: "inline-block" },
  typeCar: { background: "#dbeafe", color: "#1e40af" },
  typeUser: { background: "#fae8ff", color: "#7e22ce" },
  reason: { fontSize: 15, fontWeight: 600, color: "#111827", marginBottom: 4 },
  ...
  btnResolve: { padding: "8px 18px", background: "#16a34a", color: "#fff",
    border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" },
  btnDismiss: { padding: "8px 18px", background: "transparent",
    border: "1px solid #d1d5db", color: "#374151", borderRadius: 8,
    fontSize: 13, cursor: "pointer" },
  btnBan: { padding: "8px 18px", background: "#dc2626", color: "#fff",
    border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" },
  btnApprove: { padding: "8px 18px", background: "#16a34a", color: "#fff",
    border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" },
  btnReject: { padding: "8px 18px", background: "#dc2626", color: "#fff",
    border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" },
...
export default function Admin() {
  ...
  if (!user || user.role !== "admin") {
    return (
      <div style={s.accessDenied}>
        ...
        <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Acceso denegado</div>
        <div style={{ color: "#6b7280", marginBottom: 24 }}>No tenés permisos para ver esta página.</div>
        <button style={{ padding: "10px 24px", background: "#1d4ed8", color: "#fff",
  ...
  const suspendUser = (report) => {
    saveReports(reports.map(r => r.id === report.id ? { ...r, status: "banned" } : r));
    ...
    showAlert(`Usuario "${report.target}" suspendido. No podrá iniciar sesión.`);
  };
  ...
  const approveCar = (carId) => {
    const cars = JSON.parse(localStorage.getItem("fw_my_cars") || "[]");
    const updated = cars.map(c => c.id === carId
      ? { ...c, approved: true, is_verified: true } : c);
  ...
  const rejectCar = (carId) => {
    const cars = JSON.parse(localStorage.getItem("fw_my_cars") || "[]");
    const updated = cars.map(c => c.id === carId
      ? { ...c, rejected: true, available: false } : c);
    localStorage.setItem("fw_my_cars", JSON.stringify(updated));
    setPendingCars(updated.filter(c => !c.approved && !c.rejected));
  ...
  const ReportCard = ({ r }) => (
    <div style={s.card}>
      <div style={s.cardHeader}>
        <div>
          <span style={{ ...s.reportType, ...(r.targetType === "car" ? s.typeCar : s.typeUser) }}>
      ...
      {r.status === "pending" && (
        <div style={s.btnRow}>
          <button style={s.btnResolve} onClick={() => updateReport(r.id, "resolved")}>
            ✓ Resolver
          </button>
          <button style={s.btnDismiss} onClick={() => updateReport(r.id, "dismissed")}>
            Descartar
          </button>
          {r.targetType === "user" && (
            <button style={s.btnBan} onClick={() => suspendUser(r)}>
              🚫 Suspender usuario
            </button>
          )}
          {r.targetType === "car" && (
            <button style={{ ...s.btnBan, background: "#7c3aed" }}
              onClick={() => takeDownCar(r)}>
              🗑️ Dar de baja publicación
            </button>
          )}
        </div>
      )}
    ...
    <div style={s.page}>
      ...
      {tab === "reportes-usuarios" && (
        userReports.length === 0 ? (
          <div style={s.empty}>
            ...
            <div style={{ marginTop: 12 }}>No hay reportes de usuarios.</div>
          </div>
      ...
      {tab === "reportes-publicaciones" && (
        carReports.length === 0 ? (
          <div style={s.empty}>
            <div style={{ fontSize: 48 }}>📋</div>
            <div style={{ marginTop: 12 }}>No hay reportes de publicaciones.</div>
          </div>
      ...
      {tab === "publicaciones-pendientes" && (
        pendingCars.length === 0 ? (
          <div style={s.empty}>
            <div style={{ fontSize: 48 }}>🚗</div>
            <div style={{ marginTop: 12 }}>No hay publicaciones pendientes.</div>
          </div>
        ) : pendingCars.map(car => (
          <div key={car.id} style={s.carCard}>
            ...
            <div style={s.carInfo}>
              ...
              <div style={s.carMeta}>
                📍 {car.location} · ${Number(car.price_per_day).toLocaleString()}/día · {car.transmission} · {car.fuel}
              </div>
              ...
              {car.description && (
                <div style={{ ...s.detail, marginBottom: 12 }}>{car.description}</div>
              )}
              <div style={s.btnRow}>
                <button style={s.btnApprove} onClick={() => approveCar(car.id)}>
                  ✓ Aprobar publicación
                </button>
                <button style={s.btnReject} onClick={() => rejectCar(car.id)}>
                  ✗ Rechazar
                </button>
              </div>
            </div>
      ...
      {tab === "todos-los-autos" && (
        allCars.length === 0 ? (
          <div style={s.empty}>
            <div style={{ fontSize: 48 }}>🚗</div>
            <div style={{ marginTop: 12 }}>No hay autos en la plataforma.</div>
          </div>
        ) : allCars.map(car => (
          <div key={car.id} style={s.carCard}>
            <div style={s.carImg}>
            ...
            <div style={s.carInfo}>
              <div style={s.carTitle}>{car.brand} {car.model} {car.year}</div>
              <div style={s.carMeta}>
                📍 {car.location} · ${Number(car.price_per_day).toLocaleString()}/día · {car.transmission}
              </div>
              <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 10 }}>
                {car.owner_name
                  ? <>Publicado por <strong>{car.owner_name}</strong></>
                  : "Auto del sistema"}
                {car.banned && <span style={{ color: "#dc2626", marginLeft: 8, fontWeight: 600 }}>🚫 Dado de baja</span>}
                {car.approved && <span style={{ color: "#16a34a", marginLeft: 8, fontWeight: 600 }}>✓ Aprobado</span>}
              </div>
              <div style={s.btnRow}>
                <button style={s.btnBan}
                  onClick={() => deleteCar(car.id, `${car.brand} ${car.model} ${car.year}`)}>
                  🗑️ Eliminar de la plataforma
                </button>
              </div>
            </div>
          </div>
